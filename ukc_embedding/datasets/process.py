...
def process_dataset(path):
    ...
    return examples, filters, ent2idx, rel2idx
...
if __name__ == "__main__":
    ...
    for dataset_name in os.listdir(data_path):
        ...
        dataset_examples, dataset_filters, ent2idx, rel2idx = process_dataset(dataset_path)
        ...
        # --- START: ADDED CODE TO SAVE MAPPING FILES ---
        # 1. Save Entity mapping (name to ID)
        with open(os.path.join(dataset_path, "entity_to_id.pickle"), "wb") as save_file:
            pickle.dump(ent2idx, save_file)

        # 2. Save Relation mapping (name to ID)
        with open(os.path.join(dataset_path, "relation_to_id.pickle"), "wb") as save_file:
            pickle.dump(rel2idx, save_file)
            
        # --- END: ADDED CODE TO SAVE MAPPING FILES ---
